...
plot_mags_n <-  ggplot() + 
                ...
                labs(title="", y="De-replicated MAGs", x="") + scale_y_continuous(expand=c(0,0), limits = c(0,83000)) + preset_gen + coord_flip()  +
                scale_fill_manual(name = "MAG quality:", values = c("HQ"="#1b9e77", "MQ" = "#d95f02")) + theme(legend.position = "right")
plot_mags_n
```
...
plot_mags_n_ <- plot_mags_n +  guides(fill = guide_legend(override.aes = list(size=1),reverse=T)) + 
                theme(legend.key.size = unit(0.5, "cm"), legend.margin = unit(c(0,0,0,0), 'cm'),
                      ...
                      legend.box.background = element_blank(),legend.background = element_blank(),
                      plot.margin = unit(c(0,0,0.1,-0.3), 'cm'))
