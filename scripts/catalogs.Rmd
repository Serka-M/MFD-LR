...
plot_upset <- ComplexUpset::upset(mags_drep_,names,sort_sets=FALSE, set_sizes=FALSE, height_ratio=0.33, min_size=200, intersections='all',
              ...
              base_annotations=list('Intersection size'=intersection_size(bar_number_threshold=1, counts=T, text=list(size=2, angle=0), mapping=aes(fill=cluster_qual)) +
              stat_summary(fun = function(x) {sum_x <- sum(x); if (sum_x >= 1000) {return(sum_x)} else {return(NA)}},
                           ...
                           geom='text', position=position_stack(vjust = 0.7), size=1.75, color="white", aes(label=..y..)) +
              scale_fill_manual(values = c("MQ" = "#d95f02","HQ"="#1b9e77"), name="") + ylab("Species-level clusters") + 
              theme(axis.text.y = element_text(size = 6), axis.title.y = element_text(size = 7, margin = margin(r =-2)),  plot.margin = unit(c(0,0,-2,0), 'cm'),legend.position = "none"))) +
              ...
              labs(x="")
plot_upset
#write.table(mags_drep_,"../analysis/source/Fig_3b.tsv",quote=F,row.names=FALSE,col.names=T,sep="\t")
```
...
plot_mags_n <-  ggplot() + 
                ...
                geom_text(data = mags_n_drep_, aes(label = Total, y = Total, x= Group.2), size=2, position = position_stack(vjust = 1), color="black", hjust = -0.25) +
                geom_text(data=mags_n_drep, aes(label = x_, y = x, x=Group.2, fill=Group.1), size=1.75, position = position_stack(vjust = 0.7), color="white") +
                labs(title="", y="De-replicated MAGs", x="Genome catalogue") + scale_y_continuous(expand=c(0,0), limits = c(0,83000)) + preset_gen + coord_flip()  +
                scale_fill_manual(name = "MAG quality:", values = c("HQ"="#1b9e77", "MQ" = "#d95f02")) + 
                theme(legend.position = "right", axis.title.y = element_text(margin = margin(r = -1)))
plot_mags_n

mags_n_df <- merge(mags_n_drep,mags_n_drep_,by="Group.2")
#write.table(mags_n_df,"../analysis/source/Fig_3a.tsv",quote=F,row.names=FALSE,col.names=T,sep="\t")
```
...
plot_rrna
#write.table(ssu[,c("group","type","rrna_all","rrna_operon","frac")],"../analysis/source/Fig_3c.tsv",quote=F,row.names=FALSE,col.names=T,sep="\t")
```
...
plot_bgc
#write.table(as_filt[,c("group","type","bgc_sum","bgc","edge_frac")],"../analysis/source/Fig_3e.tsv",quote=F,row.names=FALSE,col.names=T,sep="\t")
```
...
plot_df
#write.table(df_c_agr[,c("group","type","di_all","di_full","frac")],"../analysis/source/Fig_3d.tsv",quote=F,row.names=FALSE,col.names=T,sep="\t")
```
...
plot_upset_ <- as.ggplot(plot_upset) + theme(plot.margin = unit(c(0,-0.2,-0.6,0.1), 'cm'))

#plot_mags_n_ <- plot_mags_n + guides(fill = guide_legend(override.aes = list(size=1),reverse=T)) +
#                theme(legend.key.size = unit(0.25, "cm"),legend.margin = unit(c(-3,0,0,0), 'cm'),
...
plot_mags_n_ <- plot_mags_n +  guides(fill = guide_legend(override.aes = list(size=1),reverse=T)) + 
                theme(legend.key.size = unit(0.5, "cm"), legend.margin = unit(c(0,0,0,0), 'cm'),
                      legend.box.margin = unit(c(-0.3, 0, 0, 0), 'cm'),
                      legend.box.background = element_blank(),legend.background = element_blank(),
                      plot.margin = unit(c(0,0,0.1,0), 'cm'))

fig_ab <- ggarrange(plot_mags_n_, plot_upset_, nrow=1, ncol=2, widths = c(1,1.5), common.legend = FALSE, legend="bottom", 
                    align = c("none"), labels = c("a","b"), font.label = list(size = 8))
...
plot_bgc_ <- plot_bgc + margins + theme(axis.text.y=element_blank(), plot.margin = unit(c(-0.2,0.2,-0.6,0), 'cm'))
plot_df_ <- plot_df + margins + theme(axis.text.y=element_blank(), plot.margin = unit(c(-0.2,0,-0.6,0), 'cm'))
plot_rrna_ <- plot_rrna + margins + theme(plot.margin = unit(c(-0.2,0,-0.6,0.1), 'cm')) + labs(y="Genome catalogue")

fig_ce <- ggarrange(plot_rrna_,plot_df_,plot_bgc_, nrow=1, ncol=3, widths = c(1.15,1,1.5), common.legend = TRUE, legend="bottom", 
                    align = c("h"), labels = c("c","d", "e"), font.label = list(size = 8))
...
sylph_sr_df <- as.data.frame(plot_sylph_sr$data)
#write.table(sylph_sr_df[,c("fieldsample_barcode","group","mfd_sampletype","class")],"../analysis/source/Fig_3f.tsv",quote=F,row.names=FALSE,col.names=T,sep="\t")

fig_cf <- ggarrange(fig_ce, plot_sylph_sr, nrow=1, ncol=2, widths = c(1.5,1), common.legend = FALSE, legend="bottom",
                    align = c("none"), labels = c("", "f"), font.label = list(size = 8))
