...
```{r, include=TRUE}
mags_n_drep <- aggregate(mags_drep$bin,by=list(mags_drep$MAG_status,mags_drep$group), FUN=length)
...
plot_upset_ <- as.ggplot(plot_upset) + theme(plot.margin = unit(c(-0.15,-1.8,-0.7,-0.25), 'cm'))

#plot_upset_ <- ggarrange(plot_upset_, as_ggplot(get_legend(plot_mags_n_drep+guides(fill = guide_legend(override.aes = list(size=1),reverse=T))+
...
#                         nrow=1, ncol=2, widths = c(1,0.2), align = "none")
...
plot_sylph_sr <- readRDS("../data/catalogs/sylph.rds") + theme(plot.margin = unit(c(-0.2,0.1,0,0.1), 'cm'))

fig_cf <- ggarrange(fig_ce, plot_sylph_sr, nrow=1, ncol=2, widths = c(1.5,1), common.legend = FALSE, legend="bottom",
                    align = c("h"), labels = c("", "f"), font.label = list(size = 9))
