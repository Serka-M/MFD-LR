...
plot_core_area <- ggplot(data=core_hab_sum[core_hab_sum$group=="Sample area",], aes(x=factor(type, levels=unique(type)), y=genera_perc, color=type)) + 
                  labs(title="", y="Coverage of core genera for sample area category (%)", x="Sample type") + preset
...
plot_core_hab1 <- ggplot(data=core_hab_sum[core_hab_sum$group=="Habitat (level 1)",], aes(x=factor(type, levels=unique(type)), y=genera_perc, color=type)) + 
                  labs(title="", y="Coverage of core genera for habitat descriptor (level 1, %)", x="Sample type") + preset
...
plot_core_hab2 <- ggplot(data=core_hab_sum[core_hab_sum$group=="Habitat (level 2)",], aes(x=factor(type, levels=unique(type)), y=genera_perc, color=type)) + 
                  labs(title="", y="Coverage of core genera for habitat descriptor (level 2, %)", x="Sample type") + preset
...
plot_core_hab3 <- ggplot(data=core_hab_sum[core_hab_sum$group=="Habitat (level 3)",], aes(x=factor(type, levels=unique(type)), y=genera_perc, color=type)) + 
                  labs(title="", y="Coverage of core genera for habitat descriptor (level 3, %)", x="Sample type") + preset 
...
#ggsave(plots_core_hab, file="../analysis/figures/FigS9.png", height = 90, width = 180,  units ="mm", dpi = 450, bg = "white")
```
...
```{r, include=TRUE}
preset_sylph <- list(labs(title="", y="Short reads classified (%)", color="Database:"),
                     geom_quasirandom(size = 0.01, alpha=0.4, dodge.width = 0.9),
                     geom_boxplot(outlier.shape=NA, fill=NA, width=0.3, alpha = 0.4, lwd=0.1, position = position_dodge(width = 0.9), color="black"), 
...
fig_sylph_hab_ <- ggarrange(plot_sylph_sr_sed,plot_sylph_sr_water, nrow=1, ncol=2, common.legend = TRUE, legend="bottom", 
                            align = c("h"), labels = c("b","c"), font.label = list(size = 8))
...
#ggsave(fig_sylph_hab, file="../analysis/figures/FigS10.png", height = 180, width = 200,  units ="mm", dpi = 450, bg = "white")
```
...
map <- mapDK(detail = 'region', map.colour = "grey50", map.fill = "grey98") + 
       ...
       theme(legend.key=element_blank(), legend.position = c(0.75, 0.85),
             ...
             plot.margin = margin(-5, -2, 2, 0),
             axis.text.x = element_text(size = 6, color = "black"),
             axis.text.y = element_text(size = 6, color = "black"),
             axis.ticks = element_line(color = "black")) +
       coord_cartesian(xlim = c(8, 15.25), ylim = c(54.5, 57.9), expand = FALSE) +
       scale_x_continuous(breaks = seq(8, 15, by = 2),labels = function(x) paste0(x, "°E")) +
...
```{r, include=TRUE}
# First half
plot_habs_ab <- ggarrange(map,plot_pcoa,widths=c(1.25,1), nrow=1, ncol=2, align = "none", common.legend=FALSE, legend = "none",
                          labels = c("a","b"), font.label = list(size = 8))

plot_habs_ab_leg <- as_ggplot(get_legend(plot_pcoa)) + theme(plot.margin = margin(-30, 0, 0, 0))
plot_habs_ab <- ggarrange(plot_habs_ab, plot_habs_ab_leg, nrow=2, align = "none", heights=c(1,0.4)) + theme(plot.margin = margin(0, 0, -30, 0))
...
plot_cf <- ggarrange(plot_abund_phyl,plot_df, nrow=1, ncol=2, align = "hv", common.legend=FALSE, legend="none",
                     labels = c("c",""), font.label = list(size = 8), widths=c(1,1.1)) + theme(plot.margin = margin(5, 0, 0, 0))
...
#ggsave(plot_af, file="../analysis/figures/FigS5.png", height = 200, width = 180, units ="mm", dpi = 450, bg = "white")
```
