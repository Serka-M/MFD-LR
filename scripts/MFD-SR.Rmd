...
sylph_gtdb <- read.delim("../data/MFD-SR/sylph-sum-sr-gtdb.tsv", sep="\t", header=F)
sylph_gtdb$group <- "GTDB"

sylph_gtdb_pub <- read.delim("../data/MFD-SR/sylph-sum-sr-pub.tsv", sep="\t", header=F)
sylph_gtdb_pub$group <- "GTDB + Public catalogues"

sylph_gtdb_pub_lr <- read.delim("../data/MFD-SR/sylph-sum-sr-pub-mfdlr.tsv", sep="\t", header=F)
...
sylph_sr <- merge(sylph_sr,meta,by="lib_id")

sylph_sr <- sylph_sr[sylph_sr$after_total_bases >= 10**9,]
...
#ggsave(plot_af, file="../analysis/figures/FigS5_.pdf", height = 200, width = 180, useDingbats=FALSE,  units ="mm", dpi = 300)
```
