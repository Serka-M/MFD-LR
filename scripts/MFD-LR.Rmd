...
```{r, include=TRUE}
ideel <- read.delim("../data/MFD-LR/ideel_sub.tsv", sep="\t", header=F)
...
mags_ideel <- mags_ideel[mags_ideel$rep_status=="TRUE",]
```


### Plot IDEEL scores
```{r, include=TRUE}
mags_ideel$gtdb_status2 <- factor(mags_ideel$gtdb_status2,levels=c("Known species","Unclassified species","Unclassified genus","Unclassified family"))

plot_ideel <- ggplot(data=mags_ideel,aes(x=cov,y=ideel_frac*100,col=gtdb_status2)) + 
              geom_point(alpha=0.6, size=0.8) + 
              scale_color_manual(name = "MAG representation in GTDB R220:", 
                                 values = c("Known species" = "#045a8d", "Unclassified species"="#a6dba0", "Unclassified genus" = "#fee090", "Unclassified family" = "#C4451C")) +
              scale_x_continuous(breaks=c(10,20,40,80,160,320,640,1280),trans = "log2", limits=c(10,1400),expand=c(0,0)) +
              scale_y_continuous(breaks=c(0,20,40,60,80,100),limits=c(0,102), expand=c(0,0)) +
              labs(y="IDEEL score (%)", x="MAG coverage") + preset_gen +
              guides(colour = guide_legend(override.aes = list(size=1.5), nrow=2, byrow=TRUE)) +
              theme(legend.position = "bottom", legend.key.height=unit(0.1, 'cm'), legend.key.width=unit(0.1, 'cm'))
plot_ideel

#ggsave(file="../analysis/figures/IDEEL.pdf", height = 90, width = 120, useDingbats=FALSE,  units ="mm", dpi = 300)
```
