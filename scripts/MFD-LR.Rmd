...
sankey <- ggplot(gen_sankey_, aes(x = x, next_x = next_x, node = node, next_node = next_node,
          ...
          geom_sankey_label(size = 1.5, color = "black", fill= "white",hjust=0, alpha=0.85, width=2) +  theme_bw() + coord_cartesian(clip = "off") +
          theme(axis.title = element_blank(), axis.text.y = element_blank(), axis.ticks = element_blank(),
                panel.grid = element_blank(), legend.position = "none",
                axis.text.x = element_text(margin = margin(t=-25),size = 7), panel.border = element_blank(),
                plot.margin=grid::unit(c(-8,0,0,-24), "mm"))  +
...
sankey
#ggsave(file="../analysis/figures/FigS1.pdf", height = 200, width = 160, useDingbats=FALSE,  units ="mm", dpi = 300)
#ggsave(file="../analysis/figures/FigS1.png", height = 200, width = 160,  units ="mm", dpi = 450, bg = "white")
```
...
map <- mapDK(detail = 'region', map.colour = "grey50", map.fill = "grey98") +
       ...
       theme(legend.key=element_blank(), legend.position = c(0.8, 0.75),
       ...
       coord_cartesian(xlim = c(8, 16), ylim = c(54.5, 58), expand = FALSE) +
       scale_x_continuous(breaks = seq(8, 16, by = 2),labels = function(x) paste0(x, "°E")) +
       scale_y_continuous(breaks = seq(54, 58, by = 1), labels = function(y) paste0(y, "°N")) +
       labs(x = "Longitude", y = "Latitude", col = "Habitat:")
map
#write.table(gen[,c("fieldsample_barcode","longitude","latitude","mfd_hab1_")],"../analysis/source/Fig_1a.tsv",quote=F,row.names=FALSE,col.names=T,sep="\t")
```
...
#write.table(gen[,c("fieldsample_barcode","reads_size_bp","mfd_hab1_")],"../analysis/source/Fig_1b.tsv",quote=F,row.names=FALSE,col.names=T,sep="\t")
...
#write.table(gen[,c("fieldsample_barcode","reads_N50_bp","mfd_hab1_")],"../analysis/source/Fig_1c.tsv",quote=F,row.names=FALSE,col.names=T,sep="\t")

plot_asm <- ggplot(data=gen, aes(x=name1, y=assembled)) + labs(title="", y="Sequenced data asssembled (%)", x="") +
            preset_fig1 + plot_scale
#write.table(gen[,c("fieldsample_barcode","assembled","mfd_hab1_")],"../analysis/source/Fig_1d.tsv",quote=F,row.names=FALSE,col.names=T,sep="\t")

plot_bin <- ggplot(data=gen, aes(x=name1, y=r_abund*(assembled/100))) + labs(title="", y="Sequenced data binned (%)", x="") +
            preset_fig1 + plot_scale
#write.table(gen[,c("fieldsample_barcode","assembled","r_abund","mfd_hab1_")],"../analysis/source/Fig_1e.tsv",quote=F,row.names=FALSE,col.names=T,sep="\t")
...
#write.table(gen[,c("fieldsample_barcode","HQ_mags","MQ_mags","mfd_hab1_")],"../analysis/source/Fig_1f.tsv",quote=F,row.names=FALSE,col.names=T,sep="\t")
```


### Initial Figure 1
```{r, include=TRUE}
map_ <- map + theme(plot.margin = unit(c(0.2, 0.2, 0.2, 0.4), "cm"))

fig1_bf <- ggarrange(plot_yield, plot_n50, plot_asm, plot_mags, plot_bin,
                     nrow=5, ncol=1, legend = "none", common.legend = FALSE,
                     labels = c("b","c","d","e","f"), font.label = list(size = 8))

fig1 <- ggarrange(map_, fig1_bf, nrow=1, ncol=2, common.legend = FALSE, widths=c(2,0.9),
                  align = c("none"), labels = c("a"), font.label = list(size = 8))
fig1

#ggsave(file="../analysis/figures/Fig1_pre.pdf", height = 100, width = 180, useDingbats=FALSE,  units ="mm", dpi = 300, bg = "white")
```
...
plot_gb <- ggplot(data=gen, aes(x=mfd_hab1_, y=reads_size_bp/10**9, color=mfd_hab1_)) + 
           labs(title="", y="Sequenced yield (Gbp)", x="Sample habitat") + preset_hab

plot_asm <- ggplot(data=gen, aes(x=mfd_hab1_, y=assembled, color=mfd_hab1_)) + 
            labs(title="", y="Data assembled (%)", x="Sample habitat") + preset_hab

plot_count <- ggplot(data=gen, aes(x=mfd_hab1_, y=(HQ_mags+MQ_mags), color=mfd_hab1_)) + 
              labs(title="", y="MAGs recovered (HQ and MQ)", x="Sample habitat") + preset_hab +
...
plot_bind <- ggplot(data=gen, aes(x=mfd_hab1_, y=round(r_abund*assembled/100,0), color=mfd_hab1_)) + 
             labs(title="", y="Data binned in MAGs (%)", x="Sample habitat") + preset_hab +
...
plots_mag_hab <- ggarrange(plot_gb, plot_asm, plot_count, plot_bind, nrow=2, ncol=2, align = c("v"),
                           legend = "none", common.legend = FALSE, labels = c("a","b", "c", "d"), font.label = list(size = 8))
plots_mag_hab  

#ggsave(file="../analysis/figures/FigS3.pdf", height = 150, width = 180, useDingbats=FALSE,  units ="mm", dpi = 300)
#ggsave(file="../analysis/figures/FigS3.png", height = 150, width = 180,  units ="mm", dpi = 450, bg = "white")
```
...
plot_mag_drep <- ggarrange(plot_cm2,plot_n50_cov,plot_cd_cov,plot_cd_gc,plot_cd_hp,plot_gc_hp, nrow=3, ncol=2, 
                           legend = "bottom", common.legend = TRUE, labels = c("a","b","c","d","e","f"), font.label = list(size = 8))
plot_mag_drep

#ggsave(file="../analysis/figures/FigS6.pdf", height = 200, width = 180, useDingbats=FALSE,  units ="mm", dpi = 300)
#ggsave(file="../analysis/figures/FigS6.png", height = 200, width = 180,  units ="mm", dpi = 450, bg = "white")
```
...
mags$sankey1 <- ifelse(mags$MAG_status2 == "cMAG", "Circular",
                       ifelse(mags$Total_Contigs == 1, "Single-contig",
                              ifelse(mags$Total_Contigs <= 10,"10 or less\ncontigs",
                                     ifelse(mags$Total_Contigs <= 25,"25 or less\ncontigs",
                                            ifelse(mags$Total_Contigs <= 50,"50 or less\ncontigs","More than 50\ncontigs")))))
...
mags$gtdb_status <- ifelse(mags$phylum == "Unclassified", "Unassigned phyla", 
                           ifelse(mags$class == "Unclassified", "Unassigned class",
                                  ifelse(mags$order == "Unclassified", "Unassigned order",
                                         ifelse(mags$family == "Unclassified", "Unassigned\nfamily",
                                                ifelse(mags$genus == "Unclassified", "Unassigned\ngenus",
...
mags_sankey_$node <- factor(mags_sankey_$node,levels =
                              c("More than 50\ncontigs","50 or less\ncontigs","25 or less\ncontigs","10 or less\ncontigs",
                                ...
                                "Unassigned\nfamily","Unassigned order","Unassigned class"))
...
sankey <- ggplot(mags_sankey_, aes(x = x, next_x = next_x, node = node, next_node = next_node,
          ...
          geom_sankey_label(size = 2, color = "black", fill= "white",hjust=0, alpha=0.85, width=2.25) +  theme_bw() +
          theme(axis.title = element_blank(), axis.text.y = element_blank(), axis.ticks = element_blank(),
...
sankey
#write.table(mags[,c("bin","sankey1","sankey2","sankey3","sankey4")],"../analysis/source/Fig_2a.tsv",quote=T,row.names=FALSE,col.names=T,sep="\t")
```
...
plot_ideel <- ggarrange(plot_ideel_trbl,plot_ideel_gtdb,plot_ideel_corr,plot_ideel_leg, nrow=2, ncol=2, align = "hv", common.legend=FALSE, legend = "none",
                         labels = c("a","b","c",""), heights=c(1,1.2), font.label = list(size = 8))
...
#write.table(mags[mags$rep_status == TRUE,c("bin","Contig_N50","MAG_status")],"../analysis/source/Fig_2e.tsv",quote=F,row.names=FALSE,col.names=T,sep="\t")
...
#write.table(mags[mags$rep_status == TRUE,c("bin","cov","MAG_status")],"../analysis/source/Fig_2d.tsv",quote=F,row.names=FALSE,col.names=T,sep="\t")
...
#write.table(mags[mags$rep_status == TRUE,c("bin","Coding_Density","MAG_status")],"../analysis/source/Fig_2f.tsv",quote=F,row.names=FALSE,col.names=T,sep="\t")
...
#write.table(mags[mags$rep_status == TRUE,c("bin","Genome_Size","MAG_status")],"../analysis/source/Fig_2c.tsv",quote=F,row.names=FALSE,col.names=T,sep="\t")
```
...
drep_rf_df <- as.data.frame(drep_rf_$iNextEst)
...
#write.table(drep_rf_df[,1:3],"../analysis/source/Fig_2b.tsv",quote=F,row.names=FALSE,col.names=T,sep="\t")
```


### Make Figure 2
```{r, include=TRUE}
core_gen <- readRDS("../data/MFD-LR/core_genera.rds") + labs(fill="MIMAG ranking:", x="Metadata category") + scale_x_discrete(guide = guide_axis(angle = 30)) + 
...
#write.table(core_gen$data,"../analysis/source/Fig_2g.tsv",quote=F,row.names=FALSE,col.names=T,sep="\t")

drep_rf2_ <- as.ggplot(drep_rf2) + theme(plot.margin = unit(c(0, -3, 0, -1), "mm"))
sankey_ <- sankey + theme(plot.margin = unit(c(0, 0, 0, -21), "mm"))

fig2_ab <- ggarrange(sankey_, drep_rf2_, nrow=1, ncol=2, legend = "none", align = "none",
                     common.legend = FALSE, labels = c("a","b"), font.label = list(size = 8), widths=c(1.2,1))

fig2_cf <- ggarrange(plot_mag_size, plot_mag_cov, plot_mag_n50, plot_mag_cd, nrow=2, ncol=2, legend = "none", align = "hv",
                     common.legend = FALSE, labels = c("c","d","e","f"), font.label = list(size = 8)) 

fig2_cfg <- ggarrange(fig2_cf, core_gen, common.legend = FALSE, nrow=1, ncol=2, align = "v",
                      labels = c(NA,"g"), font.label = list(size = 8), widths=c(1.15,1)) +
...
drep_rf_hab_plot <- ggiNEXT(drep_rf_hab_, color.var="Assemblage") + theme_bw(base_size = 5) + 
                    labs(x="MAGs",y="Different microbial species", linetype="", fill="", color="Habitat:") +
                    ...
                    theme(plot.margin = unit(c(2, 4, 0, 2), "mm"), legend.key.height=unit(0.3, 'cm'), legend.key.width=unit(0.25, 'cm'))
...
#ggsave(drep_rf_hab_plot3, file="../analysis/figures/FigS7.png", height = 120, width = 140, units ="mm", dpi = 450, bg = "white")
```
...
plot_drep_n <- ggplot(data=mags_drep_n2, aes(x=factor(id,level=rev(id)), y=drep_cluster_n, color=phylum)) + 
               labs(title="", y="MAGs per cluster", x="Genus (GTDB) and species cluster", color="Phylum:") + 
               ...
               preset_gen2 + theme(legend.position = "right",legend.spacing.y = unit(0.1, "cm"), legend.key.height = unit(0.4, "cm")) + 
...
#ggsave(file="../analysis/figures/FigS8.png", height = 180, width = 160,  units ="mm", dpi = 450, bg = "white")
```
...
#ggsave(drep_rf_phyl_plot2, file="../analysis/figures/FigS15.pdf", height = 1+5, width = 140, useDingbats=FALSE,  units ="mm", dpi = 300)
#ggsave(drep_rf_phyl_plot2, file="../analysis/figures/FigS15.png", height = 145, width = 140, units ="mm", dpi = 450, bg = "white")
```
...
plot_phylum <- ggplot(data = phylum_count_all2, mapping = aes(x = Group.2, y = factor(Group.1,level=level_phyla$Group.1), fill = frac)) +
               geom_tile(width=1) + scale_x_discrete(guide = guide_axis(angle = 45)) + labs(title="", y="Phylum", x="Habitat", fill="Fraction of MAGs (%):") +
...
#ggsave(file="../analysis/figures/FigS16.png", height = 180, width = 140,  units ="mm", dpi = 450, bg = "white")
```
