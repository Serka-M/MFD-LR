...
map <- mapDK(detail = 'region', map.colour = "grey50", map.fill = "grey98") + 
       ...
       scale_color_manual(name = "Highest observed taxonomic divergence:", labels=labels_n,
       ...
       coord_cartesian(xlim = c(8, 15.75), ylim = c(54.5, 57.8), expand = FALSE) +
...
def_plot_heatmap <- function(df,group_sample,group_name) {
  ...
  plot <- ggplot(data = sample_mfd, mapping = aes(x = group, y = category, fill = frac)) + geom_tile(width=1) +
          ...
          labs(title="", y=group_name, x="Taxonomic divergence level", fill="Samples with MFD-LR MAGs (%):") + 
          ...
          preset_gen2 + theme(legend.box.background = element_blank(),legend.background = element_blank(), legend.title = element_text(margin = margin(b = 1)),
                              legend.key.height = unit(0.5, "cm"), axis.title.y = element_text(margin = margin(r = -1.8)), axis.title.x = element_text(margin = margin(t = -1.8)),
                              legend.title.align = 0.5)
  
  return (plot) }

plot_count_type <- def_plot_heatmap(sylph_all,"mfd_sampletype","Sample type") + theme(plot.margin = margin(-13, 1, 2, 2))
plot_count_area <- def_plot_heatmap(sylph_all,"mfd_areatype","Area type") + theme(plot.margin = margin(-13, 1, 2, 2))
...
def_plot_dens <- function(df,x_sample,x_name) {
  plot <- ggplot() + geom_density(data=df, aes(x=.data[[x_sample]], count=x_sample, linetype=mfd_sampletype, color=mfd_areatype), size=0.6, alpha=0.7) +
          ...
          labs(linetype="Sample type:", color="Area type:", x=x_name, y="Kernel Density Estimate\n(arbitrary units)") + theme(plot.margin = margin(5, 3, 1, 12))
  return (plot) }
...
plot_den_count <- def_plot_dens(sylph_all_,"count_g","Genera with MFD-LR MAGs (count)") +
...
plot_den_abund <- def_plot_dens(sylph_all_,"abund_g","Genera with MFD-LR MAGs (cumulative abundance, %)") +
...
plot_otus_mag <- ggplot(data = stats_sylph_sub, mapping = aes(x = novelty, y = hab, fill = mags)) + 
                 ...
                 labs(title="", y="Sample type, area and habitat descriptor (level 1)", x="Taxonomic divergence level", fill="Lineages with MFD-LR\ngenomes (median):") + preset_gen2 +
...
mags_mfg$Phylum <- mfg_rank(mags_mfg$V2,mags_mfg$V3,",p:[a-zA-Z0-9 -_]+,c:",",p:",",c:",75)
```
...
```{r, include=TRUE}
plot_heatmap_leg <- as_ggplot(get_legend(plot_count_area)) + theme(plot.margin = margin(0, 3, 0, 0))

plot_heatmaps <- ggarrange(plot_count_type,plot_count_area,plot_heatmap_leg,
                           heights=c(1,1,0.3), nrow=3, ncol=1, align = "v", common.legend=FALSE, legend = "none", labels = c("b","c",""), font.label = list(size = 8))

map_ <- map + theme(plot.margin = margin(0, 1, 3, 0.5))

plot_ac <- ggarrange(map_, plot_heatmaps, nrow=1, ncol=2, widths=c(2.5,1), align = "none", labels = c("a",""), font.label = list(size = 8))
...
plot_dg <- ggarrange(plot_den_count,plot_den_abund,plot_den_g,plot_den_f, nrow=2, ncol=2, widths = c(1,1), align = "none", common.legend=TRUE, legend="bottom",
                       labels = c("d","e","f","g"), font.label = list(size = 8))

plot_ag <- ggarrange(plot_ac,plot_dg,nrow=2, heights=c(1.25,1))

#ggsave(plot_ag, file="../analysis/figures/FigS13.pdf", height=180, width=180, useDingbats=FALSE,  units="mm", dpi=300)
#ggsave(plot_ag, file="../analysis/figures/FigS13.png", height=180, width=180,  units="mm", dpi=450, bg = "white")
```
...
plot_otus_ssu <- ggplot(data = otus_all_sample, mapping = aes(x = novelty, y = hab, fill = otus)) + 
                 ...
                 labs(title="", y="Sample type, area and habitat descriptor (level 1)", x="Taxonomic divergence level", fill="Undefined lineages without\ngenomes (median):") + preset_gen2  +
...
```{r, include=TRUE}
plot_otus <- ggarrange(plot_otus_mag, plot_otus_ssu, nrow=1, ncol=2, widths = c(1,1), align = "hv", labels = c("a","b"), font.label = list(size = 8))
#ggsave(plot_otus, file="../analysis/figures/FigS14.pdf", height = 180, width = 160, useDingbats=FALSE,  units ="mm", dpi = 300)
#ggsave(plot_otus, file="../analysis/figures/FigS14.png", height = 180, width = 160,  units ="mm", dpi = 450, bg = "white")
```
