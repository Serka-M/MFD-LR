...
def_plot_heatmap <- function(df,group_sample,group_name) {
  ...
  plot <- ggplot(data = sample_mfd, mapping = aes(x = group, y = category, fill = frac)) + geom_tile(width=1) +
          ...
          labs(title="", y=group_name, x="Novelty rank", fill="Samples with novel genomes (%):") + 
          ...
          preset_gen2 + theme(legend.box.background = element_blank(),legend.background = element_blank(), legend.title = element_text(margin = margin(b = 1)),
                              legend.key.height = unit(0.5, "cm"), axis.title.y = element_text(margin = margin(r = -2)), axis.title.x = element_text(margin = margin(t = -2)),
...
plot_otus_mag <- ggplot(data = stats_sylph_sub, mapping = aes(x = novelty, y = hab, fill = mags)) + 
                 ...
                 labs(title="", y="Sample type, area and habitat descriptor (level 1)", x="Taxonomic rank", fill="Lineages with novel\ngenomes (median):") + preset_gen2 +
...
plot_otus_ssu <- ggplot(data = otus_all_sample, mapping = aes(x = novelty, y = hab, fill = otus)) + 
                 ...
                 labs(title="", y="Sample type, area and habitat descriptor (level 1)", x="Taxonomic rank", fill="Undefined lineages without\ngenomes (median):") + preset_gen2  +
