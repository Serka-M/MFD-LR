...
gtdb_plot <- function(count,label,dotsize) {
  ...
  labs(y=paste("Species per",label,sep=" "), x="Taxa queue position by species count in decreasing order") +
  preset_gen + guides(color = guide_legend(override.aes = list(size = 1))) +
  theme(legend.spacing.x = unit(0.2, "cm"), legend.key.width = unit(0.1, "cm")) +
  ...
  scale_color_manual(name = "Lineage status:", values=c("gtdb_species"="#bf812d","total_mags"="#35978f", "zero"="#bdbdbd"),
                     labels=c("gtdb_species"="Genomes in GTDB","total_mags"="GTDB + MFD-LR genomes", "zero"="MFD-LR MAGs not recovered for group"))
...
plot_cov <- ggarrange(plot_phylum,plot_class,plot_order,plot_family, labels = c("a","b","c","d"), font.label = list(size = 8),
...
#ggsave(plot_cov, file="../analysis/figures/FigS17.png", height = 240, width = 180, units ="mm", dpi = 450, bg = "white")
```
...
tree_df_ <- gtdb_tree %>% as.treedata %>% as_tibble
label_phyla$domain <- ifelse(label_phyla$label %in% tree_df_[tree_df_$group == 0,]$label,"Bacteria","Archaea")
#write.table(label_phyla,"../analysis/source/Fig_4.tsv",quote=F,row.names=FALSE,col.names=T,sep="\t")
```
...
#ggsave(plot_tree2, file="../analysis/figures/FigS11.png", height = 120, width = 180, units ="mm", dpi = 450, bg = "white")
```
...
plot_wf <- ggarrange(plot_wf_bac,plot_wf_arc, nrow=2, ncol=1,common.legend = FALSE, labels = c("a","b"), align = c("h"), font.label = list(size = 8))

#ggsave(plot_wf, file="../analysis/figures/FigS12.pdf", height = 180, width = 180, units ="mm", dpi=300, useDingbats=FALSE)
#ggsave(plot_wf, file="../analysis/figures/FigS12.png", height = 180, width = 180, units ="mm", dpi=450)
```
